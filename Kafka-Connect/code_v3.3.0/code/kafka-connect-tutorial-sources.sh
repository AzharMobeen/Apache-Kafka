...
# Linux / Mac
docker run --rm -it -v "$(pwd)":/tutorial --net=host landoop/fast-data-dev:2.6.2 bash
# Windows Command Line:
docker run --rm -it -v %cd%:/tutorial --net=host landoop/fast-data-dev:2.6.2 bash
# Windows Powershell:
docker run --rm -it -v ${PWD}:/tutorial --net=host landoop/fast-data-dev:2.6.2 bash
...
# Read the topic data
docker run --rm -it --net=host landoop/fast-data-dev:2.6.2 bash
